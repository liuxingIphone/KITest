...
+ (NSString*)getUDIDFromKeyChain{
    ...
    if (queryErr == errSecItemNotFound) {
        KILog(@"KeyChain Item: %@ not found!!!", [NSString stringWithUTF8String:kKeychainUDIDItemIdentifier]);
    }
    else if (queryErr != errSecSuccess) {
        KILog(@"KeyChain Item query Error!!! Error code:%ld", (long)queryErr);
    }
    if (queryErr == errSecSuccess) {
        KILog(@"KeyChain Item: %@", udidValue);
...
+ (BOOL)setUDIDToKeyChain:(NSString*)udid{
    ...
    NSInteger lenth = 0;
    if (udidStr) {
        lenth = strlen(udidStr);
    }
    NSData *keyChainItemValue = [NSData dataWithBytes:udidStr length:lenth];
    ...
    if ([KKUDID getUDIDFromKeyChain]) {        // there is item in keychain
        ...
        if (writeErr != errSecSuccess) {
            KILog(@"Add KeyChain Item Error!!! Error Code:%ld", (long)writeErr);
            
            [dictForAdd release];
            return NO;
        }
        else {
            KILog(@"Add KeyChain Item Success!!!");
            [dictForAdd release];
            return YES;
        }
    }
...
+ (BOOL)removeUDIDFromKeyChain{
    ...
    if (deleteErr != errSecSuccess) {
        KILog(@"delete UUID from KeyChain Error!!! Error code:%ld", (long)deleteErr);
        [dictToDelete release];
        return NO;
    }
    else {
        KILog(@"delete success!!!");
    }
...
+ (BOOL)updateUDIDInKeyChain:(NSString*)newUDID{
    ...
    if (queryResult) {
        ...
        if (updateErr != errSecSuccess) {
            KILog(@"Update KeyChain Item Error!!! Error Code:%ld", (long)updateErr);
            ...
            KILog(@"Update KeyChain Item Success!!!");
            [dictForQuery release];
            [dictForUpdate release];
            return YES;
        }
