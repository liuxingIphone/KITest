/*
 ...
 # Author  : thomas (thomas.wang@51job.com)
 # Notes   : 网上的代码，直接拿来用的，用于正则匹配
 #####################################################################
 ### Change Logs   ###################################################
 #####################################################################
 ---------------------------------------------------------------------
...
// it registers with the default NSNotificationCenter to call the MYLLowMemoryWarningObserver class method +lowMemoryWarning:.
...
@interface      MYLLowMemoryWarningObserver : NSObject + (void)lowMemoryWarning:(id)notification; @end
@implementation MYLLowMemoryWarningObserver
+ (void)lowMemoryWarning:(id)notification {
  ...
  else { [[MYLLowMemoryWarningObserver class] performSelector:@selector(lowMemoryWarning:) withObject:notification afterDelay:(NSTimeInterval)0.1]; }
}
@end
...
__attribute__((constructor)) static void rkl_RegisterForLowMemoryNotifications(void) {
  ...
  if(didSwap == true) {
    if((memoryWarningNotification = (void **)dlsym(RTLD_DEFAULT, "UIApplicationDidReceiveMemoryWarningNotification")) != NULL) {
      [[NSNotificationCenter defaultCenter] addObserver:[MYLLowMemoryWarningObserver class] selector:@selector(lowMemoryWarning:) name:(NSString *)*memoryWarningNotification object:NULL];
    }
  }
}
  ...
  *matchLengthPtr = (NSUInteger)matchLength;
  ...
  if(((cachedRegex->setToIsImmutable == 1U) ? isSetTo : (NSUInteger)((isSetTo == 1UL) && (cachedRegex->setToLength == matchLength) && (cachedRegex->setToHash == CFHash((CFTypeRef)matchString)))) == 0UL) {
    ...
    cachedRegex->setToNeedsConversion = (cachedRegex->setToUniChar == NULL) ? 1U : 0U;
      if (rkl_CFStringIsMutable(cachedRegex->setToString) == YES) {
          cachedRegex->setToIsImmutable     = 0U;
      }
//      else{
//          cachedRegex->setToIsImmutable     = 1U;
//      }
...
exitNow:
  ...
  if((buffer.uniChar = (UniChar *)CFStringGetCharactersPtr(buffer.string)) == NULL) {
    ...
    if(RKL_EXPECTED((buffer.uniChar = (RKL_STRONG_REF UniChar * RKL_GC_VOLATILE)rkl_realloc((RKL_STRONG_REF void ** RKL_GC_VOLATILE)&buffer.uniChar, ((size_t)buffer.length * sizeof(UniChar)), 0UL)) == NULL, 0L)) { goto errorExit; } // Resize the buffer.
      if (rkl_collectingEnabled()) {
//          needToFreeBufferUniChar = 0U;
      }else{
